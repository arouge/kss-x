...
{
  kssplay->vm->cpu_speed = cpu_speed ;
//  VM_set_clock(kssplay->vm, getclk(kssplay), kssplay->vsync_freq) ;
	VM_set_clock(kssplay->vm, cpu_speed, kssplay->vsync_freq) ;
...
KSSPLAY *KSSPLAY_new(k_uint32 rate, k_uint32 nch, k_uint32 bps, int cpuSpeed)
{
  KSSPLAY *kssplay ;
  ...
  memset(kssplay,0,sizeof(KSSPLAY));
	kssplay->cpu_speed = cpuSpeed;
  if(!(kssplay->vm = VM_new(rate, cpuSpeed)))
  {
    free(kssplay) ;
    return NULL ;
  }
...
INLINE static void calc_mono(KSSPLAY *kssplay, k_int16 *buf, k_uint32 length)
{
  ...
  for(i=0;i<length;i++)
  {
    ...
    if(kssplay->kss->msx_audio&&!kssplay->device_mute[EDSC_OPL])
		d = apply_volume(OPL_calc(kssplay->vm->opl),volume[EDSC_OPL]) ;
	else 
      d = 0;

    if(kssplay->kss->fmpac&&!kssplay->device_mute[EDSC_OPLL])
		d += apply_volume(OPLL_calc(kssplay->vm->opll),volume[EDSC_OPLL]);
*/
    ...
    if(!kssplay->device_mute[EDSC_PSG])
    {
      ...
      //  d += apply_volume(SNG_calc(kssplay->vm->sng),volume[EDSC_PSG]);
      //else
          d += PSG_calc(kssplay->vm->psg);// + kssplay->vm->DA1,volume[EDSC_PSG];

//        d += apply_volume(PSG_calc(kssplay->vm->psg) + kssplay->vm->DA1,volume[EDSC_PSG]);
    }

    if(!kssplay->device_mute[EDSC_SCC])
      d += apply_volume(SCC_calc(kssplay->vm->scc) + kssplay->vm->DA8,volume[EDSC_SCC]);
...
//      c = OPL_calc(kssplay->vm->opl); 
    ...
    if(kssplay->kss->fmpac&&!kssplay->device_mute[EDSC_OPLL])
    {
      if(kssplay->opll_stereo)
      {
        ...
        ch[0] += apply_volume(b[0],volume[EDSC_OPLL][0]);
        ch[1] += apply_volume(b[1],volume[EDSC_OPLL][1]);
      }
      else
      {
		  c= OPLL_calc(kssplay->vm->opll);
    ...
    if(!kssplay->device_mute[EDSC_PSG])
    {
      if(kssplay->kss->sn76489)
	  {
		if (kssplay->kss->stereo)
		{
	        ...
	        ch[0] += apply_volume(ch[0],volume[EDSC_PSG][0]);
	        ch[1] += apply_volume(ch[1],volume[EDSC_PSG][1]); 
		}
		else
		{
			c = SNG_calc(kssplay->vm->sng);
