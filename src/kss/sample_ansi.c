/*
  ...
  KSSPLAY SAMPLE for ANSI C 
  Play KSS file and Write Wave file.
 ...
 // {
   ...
   // printf("%03d/%03d",t+1,play_time) ; fflush(stdout) ;
    ...
    /* Create 1 sec wave block */
		KSSPLAY_calc(kssplay, wavebuf, 44100) ;
		KSSPLAY_reset(kssplay, song_num, pos) ;
		printf("Plante à l'itération: %i\n",t);
		pos++;
	}
	
    /* force little endian (for sparc, powerPC, etc...) */
//    for(i=0; i<rate*2; i++) WORD((char *)(wavebuf+i), wavebuf[i]) ;
